...
class EnhancedAgentIntegration {
  ...
  Future<ToolExecutionResult> _executeToolCall(
    ...
    try {
      ...
      try {
        arguments = _safeJsonDecode(toolCall.function.arguments);
      } catch (e) {
  ...
  /// 安全 JSON 解码
  Map<String, dynamic> _safeJsonDecode(String argumentsStr) {
    try {
      if (argumentsStr.isEmpty || argumentsStr.trim() == "{}") {
        return <String, dynamic>{};
      }
      final parsed = jsonDecode(argumentsStr);
      if (parsed is Map) {
        return Map<String, dynamic>.from(parsed);
      } else if (parsed is String) {
        final reparsed = jsonDecode(parsed);
        if (reparsed is Map) {
          return Map<String, dynamic>.from(reparsed);
        }
      }
      _log.warning("工具参数类型意外", {"type": parsed.runtimeType.toString()});
      return <String, dynamic>{};
    } catch (e) {
      _log.error("参数解析失败", {"error": e.toString()});
      return <String, dynamic>{};
    }
  }
